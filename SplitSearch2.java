...
public class SplitSearch2 {
	...
	public boolean label(IntVar[] vars) {
		...
		//System.out.println("sss: " + vars[0]);
		// Do note that constraint imposition does not involve checking if the constraint is consistent.
		// if needed, so the method store.consistency can be used
		consistent = store.consistency();
		/*
		if(vars.length != 0){
			System.out.println(vars[0] + " " + vars[0].min() + " " + vars[0].max() );
