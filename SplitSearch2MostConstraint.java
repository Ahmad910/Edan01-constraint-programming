...
public class SplitSearch2MostConstraint {
	...
	public SplitSearch2MostConstraint(Store s) {
		store = s;
	}
	...
	public boolean label(IntVar[] vars) {
		...
		if (costVariable != null) {
			try {
				...
				if (costVariable.min() <= costValue - 1) {
					...
					/*
					 * System.out.println(store.level + " level");
					 * System.out.println(costVariable + " costVariable");
					 * System.out.println(costVariable.min() + " min");
					 * System.out.println(costValue - 1 + " costValue");
					 * System.out.println(
					 * "---------------------------------------------");
					 */
				} else {
		...
		if (!consistent) {
			...
			// System.out.println(wrongDecisions);
			wrongDecisions++;
			return false;
		} else { // consistent
			...
			if (vars.length == 0) {
				...
				if (costVariable != null) {
					// System.out.println(costVariable.min());
					costValue = costVariable.min();
				}

				reportSolution();
				boolean b = costVariable == null;
				// System.out.println(b + " costVariable == null");
				return b; // true is satisfiability search
	...
	void restoreLevel() {
		...
		// System.out.println(store.level + " sssaqs");
	}
	...
	public class ChoicePoint {
		...
		IntVar selectVariable(IntVar[] v) {
			int indexOfElementConstraints = 0;
			int domain = Integer.MIN_VALUE;
			int elementDomain;
			for(int i = 0; i < v.length; i++){
				elementDomain = v[i].sizeConstraints();
				if(elementDomain > domain){
					indexOfElementConstraints = i;
					domain = elementDomain;
				}
			}
			if (v.length != 0) {
				if (v[indexOfElementConstraints].min() == v[indexOfElementConstraints].max()) {
					searchVariables = new IntVar[v.length - 1];
					int j = 0;
					for (int i = 0; i < v.length - 1; i++) {
						if (indexOfElementConstraints == j) {
							j++;
							i--;
						} else {
				...
				return v[indexOfElementConstraints];
			} else {
		...
		int selectValue(IntVar v) {
			return ((v.max() + v.min()) % 2 == 0) ? ((v.max() + v.min()) / 2) 
					: ((v.max() + v.min() + 1) / 2);
		}
